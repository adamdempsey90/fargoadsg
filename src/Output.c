...
void UpdateHistory(Rho, Vr, Vt, Energy,time)
     PolarGrid *Rho, *Vr,*Vt,*Energy;
     real time;
{
/* AMD 7/7/15 
 * Create a history file that is appended to every output time.
 */
  int i,nr,ns,correctproc,alreadyroot;
  ...
  rval = GlobalRmed[GLOBALNRAD/2+1];
  correctproc = 0;
  for(i=0;i<nr;i++) {
  	if (Rmed[i] == rval) {
//		printf("\tCPU: %d, \t Match (%g,%g),\t T=%g\n",CPU_Rank,Rmed[i],rval,time);
		vrval = vr[i*ns];
		correctproc = 1;
		if (CPU_Rank == 0) {
			alreadyroot = 1;
...
/* Send vr value from appropriate processor to root */
  if (CPU_Rank != 0) {
	if (correctproc == 1) {
//		printf("CPU: %d, sending %g to root\n", CPU_Rank,vrval);
 		MPI_Send(&vrval,1,MPI_DOUBLE,0,522,MPI_COMM_WORLD);
	}
  }
  else {
	if (correctproc != 1) {
		MPI_Recv(&vrval,1,MPI_DOUBLE,MPI_ANY_SOURCE,522,MPI_COMM_WORLD,&stat1);
//		printf("CPU: %d, recieved %g\n", CPU_Rank,vrval);
	}  
  }

  if (CPU_Rank == 0) {
	sprintf(fname,"%shistory.dat",OUTPUTDIR);
	printf("Appending to history file\n");	
	if (time == 0) 
		out = fopen(fname,"w");
	else 
		out = fopen(fname,"a");
	if (out == NULL) {
		fprintf (stderr, "Can't open history.dat\n");
