...
/* AMD 7/7/15 
*  Add peturbation to inital profile here */
  AddUserIC(gas_density, gas_v_rad, gas_v_theta, gas_energy);
  ...
  for (i = begin_i; i <= NTOT; i++) {
    ...
    if (NINTERM * (TimeStep = (i / NINTERM)) == i) {
      ...
      UpdateHistory( gas_density, gas_v_rad, gas_v_theta, gas_energy,PhysicalTime);
